...
Route::prefix('auth')->group(function() {
    ...
    Route::prefix('register')->group(function() {
        Route::get('/', [\Modules\Auth\Http\Controllers\RegisterController::class , 'viewRegister'])->name('auth.register.view');
        Route::middleware('throttle:5,2')->post('store', [\Modules\Auth\Http\Controllers\RegisterController::class , 'storeRegister'])->name('auth.register.store');
        ...
        Route::middleware('throttle:4,2')->get('send/again/{token}', [\Modules\Auth\Http\Controllers\RegisterController::class , 'sendAgainCode'])->name('auth.register.send.code');
...
//        One time code
        ...
        Route::post('view-info-store', [\Modules\Auth\Http\Controllers\LoginController::class , 'viewInfoLoginStore'])->name('auth.login.information-login.store');
        Route::get('confirm/{token}', [\Modules\Auth\Http\Controllers\LoginController::class , 'viewConfirmRegister'])->name('auth.login.confirm.view');
        Route::post('confirm/store/{token}', [\Modules\Auth\Http\Controllers\LoginController::class , 'storeConfirmRegister'])->name('auth.login.confirm.store');
    });
