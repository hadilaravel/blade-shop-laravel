        ...
        <div class="bg-white shadow-xl my-5 lg:my-10 rounded-xl md:rounded-2xl p-3 md:p-5">
            <div class="flex flex-col md:flex-row gap-5">
                ...
                <div class="md:w-8/12 lg:w-9/12 flex flex-col gap-y-5">
                    <form action="{{ route('user.profile.tickets.update' , $ticket->id) }}" method="post">
                        @csrf
                        {{ method_field('put') }}
                    <div class="border rounded-3xl shadow-lg flex flex-col p-5 gap-y-5">
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" value="{{ old('title' , $ticket->title) }}" class="text-sm block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none focus:border-red-300"/>
                            @error('title')
                            <p style="color: red">
                            {{ $message }}
                            </p>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="category_id"  class="text-sm block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none focus:border-red-300">
                                ...
                                @foreach($ticketCategories as $ticketCategory)
                                    <option value="{{ $ticketCategory->id }}" @if(old('category_id' , $ticket->category_id) == $ticketCategory->id) selected @endif>{{ $ticketCategory->name }}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                            <p style="color: red">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="body" rows="10"  class="text-sm block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 font-normal text-gray-700 outline-none focus:border-red-300">{{ old('body' , $ticket->body) }}</textarea>
                            @error('body')
                            <p style="color: red">
                                {{ $message }}
                            </p>
